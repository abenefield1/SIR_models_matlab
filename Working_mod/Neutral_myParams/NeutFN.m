function dydt = NeutFN(time, y, myBeta, nu, mu, b, a, N, dMax)
    ...
    %dMax = sensitivity * testRate; % Still need to adjust testRate unless plan to model in years
        ...
        dydt(1) = b*N - b*S - myBeta * S * sum( y(2:nPlus2) );  % this is dS/dt 

        % next the equation for class 0 of the influenza
        det = dMax * exp(-0 * a);
        %det = exp(-0 * a);
        dydt(2) = myBeta * S * y(2) - (nu + mu + det + b) * y(2); % this is dI0/dt

        % the last special case is the last strain, class "n" in mathematical notation, i.e., dIn/dt:
        %DET = dMax * exp(-n * a);
        DET = exp(-n * a);
        dydt(nPlus2) = myBeta * S * y(nPlus2) - (nu + DET + b) * y(nPlus2) + mu * y(nPlus2 - 1);% dIn/dt
        ...
        for k = 1:(n-1) % iterating over all strains from strain 1 to strain n - 1
            ...
            d = exp(-k * a); % Sam added this here and changed "K" to "k"
            Ik = y(index);
            dydt(index) = myBeta * S * Ik - ((nu + mu + d +b) * Ik) + (mu * y(index - 1)); % dIk/dt
        end

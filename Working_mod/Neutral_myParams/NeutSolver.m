...
totalTime = 100;
a = 1;%0.06; % testing escape parameter - the higher a, the more rapid the escape
...
%myBeta = 0.003;
mu = 0.25;  % strain mutation parameter - definitely need to adjust, but currently in units of subs/site of the ompA C. trachomatis gene
...
for i = 1:l
    ...
    title(caption, 'FontSize', 30, 'FontName', 'Times New Roman');
    set(gca, 'FontName', 'Times New Roman')
    set(gca,'FontSize',30)
    axis([0 60 340 1000])
end
title('Time')
sgtitle('Total Force of Infection ($\sum_{k=0}^{n}I_{k}$)', 'Interpreter','latex', 'FontSize', 30, 'FontName', 'Times New Roman')

%'($d_{k=#%d}^{n}I_{k}$)', 'Interpreter','latex'

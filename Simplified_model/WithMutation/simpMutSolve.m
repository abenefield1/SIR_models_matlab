...
[t, class]=ode45(@(t, class) simpMut(t, class, N, beta, nu, b, D, det), T1,[S_0 I_0 R_0]);
S=class(:,1);
I=class(:,2);
R=class(:,3);
...
for i = 1:n
    det = DetVec(i);
    [t, class2]=ode45(@(t, class) simpMut(t, class, N, beta, nu, b, D, det), T2, class(size(class,1),:) );
    S=class2(:,1);
    I=class2(:,2);
    R=class2(:,3);
...
suplabel('Years');
hL = legend([p2],{'Infected (I)'}, 'Orientation', 'horizontal');
newPosition = [0.4 0.87 0.2 0.2];
newUnits = 'normalized';
set(hL,'Position', newPosition,'Units', newUnits, 'color','none','Box','off');
